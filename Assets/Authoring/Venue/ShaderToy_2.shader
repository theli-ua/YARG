...
fixed4 mainImage( vec2 fragCoord )
{
    ...
    float fft  = tex2D(_Yarg_SoundTex, vec2(0.0,0.0)).r;
    fft = pow(fft, .8);
    float aa = abs(atan(uv.y, uv.x))/10.+fft*3.;
    float lod = 100.;
    aa = floor(aa*lod)/lod;
    //float fft2 = texture(iChannel0, vec2(fract(aa),0)).r;
    float fft2  = tex2D(_Yarg_SoundTex, vec2(fract(aa),0)).r;
    ...
    float index = floor(t);
    float anim = fract(t);
    rnd = mix(random(vec2(index, index)), random(vec2(index+1., index+1.)), anim);
    ...
    // glitch blur
    vec2 llod = 10.*vec2(random(vec2(floor(time*4.+.5), floor(time*4.+.5))), random(vec2(floor(time*2.), floor(time*2.))));
